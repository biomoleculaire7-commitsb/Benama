...
def add_summons(national_id, last_name, first_name, class_name, requested_by_role, requested_by_name, on_behalf_of, reason, date, time):
    conn = get_conn(); cur = conn.cursor()
    cur.execute("""INSERT INTO summons (national_id, last_name, first_name, class_name,
                    requested_by_role, requested_by_name, on_behalf_of, reason, summon_date, summon_time)
                    VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)""",
                (national_id, last_name, first_name, class_name, requested_by_role, requested_by_name, on_behalf_of, reason, date, time))
    conn.commit(); cur.close(); conn.close()
...
def get_summons_for_date(date):
    conn = get_conn(); cur = conn.cursor()
    cur.execute("""SELECT national_id, last_name, first_name, class_name AS "class",
                    requested_by_role, requested_by_name, on_behalf_of, reason,
...
def get_summons_for_student(national_id):
    conn = get_conn(); cur = conn.cursor()
    cur.execute("""SELECT requested_by_role, requested_by_name, on_behalf_of, reason,
                    to_char(summon_date,'YYYY-MM-DD') AS date, summon_time AS time
